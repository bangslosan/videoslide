...
@interface SCCompositionTrackSegmentInfo : NSObject
{
@public
...
@implementation SCCompositionTrackSegmentInfo

@end

@interface SCVideoCompositionStageInfo : NSObject
{
@public
	CMTimeRange	 timeRange;
...
@implementation SCVideoCompositionStageInfo

@end

@implementation SCVideoDebugViewer
...
- (void)synchronizeToComposition:(AVComposition *)composition videoComposition:(AVVideoComposition *)videoComposition audioMix:(AVAudioMix *)audioMix
{
	...
	if (composition) {
		...
		for (AVCompositionTrack *t in composition.tracks) {
			...
			for (AVCompositionTrackSegment *s in t.segments) {
				SCCompositionTrackSegmentInfo *segment = [[SCCompositionTrackSegmentInfo alloc] init] ;
				if (s.isEmpty)
	...
	if (videoComposition) {
		...
		for (AVVideoCompositionInstruction *instruction in videoComposition.instructions) {
			SCVideoCompositionStageInfo *stage = [[SCVideoCompositionStageInfo alloc] init];
...
- (void)drawRect:(CGRect)rect
{
	...
	if (compositionTracks) {
		...
		for (NSArray *track in compositionTracks)
        {
			...
			for (SCCompositionTrackSegmentInfo *segment in track)
            {
	...
	if (videoCompositionStages) {
		...
		for (SCVideoCompositionStageInfo *stage in videoCompositionStages) {
